...
class PortfolioService:
    ...
    async def get_current_stock_price(symbol: str) -> float:
        ...
        try:
            ...
            if price_info and "regularMarketPrice" in price_info:
                ...
                print(f"yahooquery price for {symbol}: {price}")
                return price
            else:
    ...
    async def get_company_name(symbol: str) -> str:
        try:
            print(f"Trying yfinance.Ticker.info for {symbol}")
            info = yf.Ticker(symbol).info
            name = info.get("longName", "N/A")
            print(f"yfinance company name for {symbol}: {name}")
            return name
        except Exception as e:
            print(f"yfinance failed for company name: {e}")

        try:
            print(f"Trying yahooquery for company name of {symbol}")
            ticker = Ticker(symbol)
            info = ticker.price.get(symbol)
            name = info.get("longName") or info.get("shortName") or "N/A"
            print(f"yahooquery company name: {name}")
            return name
        except Exception as e:
            print(f"yahooquery failed for company name: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to fetch company name for {symbol}")
