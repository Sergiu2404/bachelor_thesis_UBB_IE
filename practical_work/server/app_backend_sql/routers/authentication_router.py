...
from data.models.user import User as UserResponseModel, Token  # Pydantic models
...
async def get_current_user(
    ...
    user = await AuthenticationService.get_user_by_username(username, db)
    if not user:
        raise HTTPException(status_code=401, detail="User not found")
    return user


@authentication_router.post("/register", response_model=UserResponseModel)
async def register(user: UserRegister, db: AsyncSession = Depends(get_db)):
    created_user = await AuthenticationService.register_user(user, db)
    return created_user


@authentication_router.post("/login", response_model=Token)
async def login(user: UserLogin, db: AsyncSession = Depends(get_db)):
    return await AuthenticationService.login_user(user, db)
...
@authentication_router.get("/connected-user", response_model=UserResponseModel)
async def get_connected_user(current_user: DBUser = Depends(get_current_user)):
    return current_user
...
async def update_balance(
    ...
    return await AuthenticationService.update_balance(current_user, balance_update, db)
